...
import styles from './v2-products.module.css';
...
export default async function V2ProductsPage({ searchParams }: PageProps) {
  ...
  const limit = 9; // 9 items per page (3x3 grid) is standard and clean
  ...
  const getCategoryLink = (catId?: number) => {
    ...
    return `/v2/products?${queryParams.toString()}`;
  };
  ...
  // Filter out system categories that do not contain our products (like Expenses, Deliveries)
  // to keep the catalog clean, but keep PMR, PROF, AVTO, KPK, Radio-Videonyani, etc.
  ...
  return (
    <div className={styles.container}>
      ...
      <div style={{ marginBottom: '20px' }}>
        <h1 style={{ fontFamily: 'Outfit', fontSize: '28px', fontWeight: '800', textTransform: 'uppercase' }}>
      ...
      <div className={styles.shopLayout}>
        ...
        <main className={styles.mainContent}>
          ...
          {/* Product Grid */}
          {products.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 20px', backgroundColor: '#f8f9fa', borderRadius: '4px', border: '1px dashed #e1e4e6' }}>
              <p style={{ color: '#A39B95', fontSize: '15px' }}>Mahsulotlar topilmadi. Qidiruv so'zini yoki filtrni o'zgartirib ko'ring.</p>
            </div>
          ) : (
            <>
                      ...
                      <div className={styles.imageWrapper}>
                        {hasImage ? (
                          ...
                          <div className={styles.noImage}>
                            ...
                            <span style={{ marginTop: '8px' }}>Rasm yo'q</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className={styles.content}>
                        ...
                        <p className={styles.description}>
                          {typeof product.description_sale === 'string'
                            ...
                            : "Aloqa sifati va qulaylikni ta'minlovchi professional vosita."}
                        </p>
                        
                        <div className={styles.footerRow}>
                          <span className={styles.price}>{formatPrice(product.list_price)}</span>
                          <button className={styles.ctaButton}>Sotib olish</button>
                        </div>
                      </div>
                    </article>
                  );
              ...
              {/* Pagination */}
              {totalPages > 1 && (
                <nav className={styles.pagination} aria-label="Shop Pagination">
                  {currentPage > 1 ? (
